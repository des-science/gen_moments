        ...
        for binx in tomo_bins:
            ...
            if not skip_conversion_toalm:
                ...
                if field_label2 != None:
                    KQU_masked_maps = [self.fields[field_label1][binx],
                                       self.fields[field_label1][binx], self.fields[field_label2][binx]]
                else:
                    KQU_masked_maps = self.fields[field_label1][binx]

                if (shear) and (len(KQU_masked_maps) == 3):
                    alms = hp.map2alm(KQU_masked_maps, lmax=lmax,
                                      pol=True)  # Spin transform!

                    # E modes
                    alms_container.append(
                        alms[1] * 1. * ((ell * (ell + 1.)) / ((ell + 2.) * (ell - 1))) ** 0.5)
                    # B modes
                    alms_container.append(
                        alms[2] * 1. * ((ell * (ell + 1.)) / ((ell + 2.) * (ell - 1))) ** 0.5)
                    ...
                    alms_container.append(hp.map2alm(
                        KQU_masked_maps, lmax=lmax, pol=False))
            ...
            for ix, almx in enumerate(alms_container):
                ...
                for j, sm in enumerate(self.conf['smoothing_scales']):
                    if self.conf['verbose']:
                        print(sm)
                    path = self.conf['output_folder']+'/smoothed_maps/' + \
                        output_label+'_'+ll[ix]+'_bin_' + \
                        str(binx)+'_sm_'+str(sm)+'.fits'
                    ...
                    if (not os.path.exists(path)) or (overwrite):
                        ...
                        B = (2*np.pi/(2.*ell+1.0))
                        fact = -B*(scipy.special.eval_legendre(ell+1, np.cos(sm_rad))
                                   - scipy.special.eval_legendre(ell-1, np.cos(sm_rad)))*A
                        fact[0] = 1
                        ...
                        # convert bak to map
                        mapaa = hp.alm2map(
                            alms1, nside=self.conf['nside'], lmax=self.conf['lmax'], pol=False)
                        del alms1
                        ...
                        if not skip_loading_smoothed_maps:
                            ...
                            mapaa = None
                    if not skip_loading_smoothed_maps:
                        self.smoothed_maps[output_label+'_'
                                           + ll[ix]][binx][sm] = copy.deepcopy(mapaa)
                        del mapaa
            del alms_container
            gc.collect()
        ...
        if not do_all_matrix:
            for i, bin1 in enumerate(tomo_bins1):
                for j, bin2 in enumerate(tomo_bins2):
                    if i != j:
                        if j < i:
                            moments_mute['{0}_{1}'.format(bin1, bin2)] = copy.deepcopy(
                                moments_mute['{1}_{0}'.format(bin1, bin2)])
        ...
        for i, bin1 in enumerate(tomo_bins1):
            for j, bin2 in enumerate(tomo_bins2):
                for zxk, bin3 in enumerate(tomo_bins2):
                    ...
                    if compute_it:
                        moments_3 = np.zeros(
                            len(self.conf['smoothing_scales']))
                        for kk, sm in enumerate(self.conf['smoothing_scales']):
        ...
        for i, bin1 in enumerate(tomo_bins1):
            for j, bin2 in enumerate(tomo_bins2):
                for zxk, bin3 in enumerate(tomo_bins2):
                    ...
                    if (zxk < j) and (j >= i):
                        moments_mute['{0}_{1}_{2}'.format(bin1, bin2, bin3)] = copy.deepcopy(
                            moments_mute['{0}_{1}_{2}'.format(binss[ordi[0]], binss[ordi[1]], binss[ordi[2]])])
                    if not do_all_matrix:
                        if (zxk >= j) and (j < i):
                            moments_mute['{0}_{1}_{2}'.format(bin1, bin2, bin3)] = copy.deepcopy(
                                moments_mute['{0}_{1}_{2}'.format(binss[ordi[0]], binss[ordi[1]], binss[ordi[2]])])

...
class theory_setup():
    ...
    def __init__(self, options):
        ...
        self.fsky = options.get_double(option_section, "fsky", default=0.11789)
        self.sm_all = ast.literal_eval(
            options.get_string(option_section, "sm_all", default='[21.0,33.6,54.,86., 137.6, 220.16]')
            )
        self.source_zbins = ast.literal_eval(options.get_string(option_section, "source_zbins", default='[1,2,3,4]'))
        self.do_mask_stuff = options.get_bool(option_section, "do_mask_stuff", default=True)
        self.nside_pixwin = options.get_int(option_section, "nside_pixwin", default=512)
        self.fname_mask_stuff = options.get_string(
            option_section,
            "fname_mask_stuff",
            default=os.environ['COSMOSIS_SRC_DIR'] + '/gen_moments/namaster_stuff.pk'
            )
        self.scheme = options.get_string(option_section, "scheme", default='GM')
        self.IA_model = options.get_string(option_section, "IA_model", default='new')
        ...
        self.id_corr2_unique, self.id_kp2_unique, self.id_corr3_unique, self.id_kp3_unique = get_unique_ind.save_unique_combs(
            self.nsm, self.nbins_tot_inp
            )

    def get_Pk_camb(self, block):
        Om, h, Ob, ns = block[cosmo_sec, "omega_m"], block[cosmo_sec, "h0"], block[cosmo_sec,
                                                                                   "omega_b"], block[cosmo_sec, "n_s"]
        ...
        bestfit["mnu"] = mnu
        bestfit["nnu"] = nnu
        bestfit["halofit_version"] = 'takahashi'
        # bestfit["halofit_version"] = 'mead2020_feedback'
        ...
        self.PK = camb.get_matter_power_interpolator(
            pars_LCDM, hubble_units=False, k_hunit=False, kmax=50.0, zmax=4, nonlinear=True, extrap_kmax=10**10
            )
        self.PK_L = camb.get_matter_power_interpolator(
            pars_LCDM, hubble_units=False, k_hunit=False, kmax=50.0, zmax=4, nonlinear=False, extrap_kmax=10**10
            )
        ...
        for jz in range(nz):
            ...
            delta_ns = 0.00001
            ns = (kay / self.PK_L.P(0, kay)) * (self.PK_L.P(0, kay + delta_ns) - self.PK_L.P(0, kay)) / (delta_ns)
            self.ns_mat[jz, :] = ns
        return 0
    ...
    def get_qigrav_and_nz(self, block):
        ...
        if self.nbins_tot_inp != self.nbins_tot:
            raise ValueError(
                "Input number of source bins in the ini file differ from what I could find in the 2pt file. This will give error in getting unique combination of the bins"
                )
    ...
    def set_IAz(self, block):
        z0 = block[IA_sec, 'z_piv']
        AIA0 = block[IA_sec, 'A1']
        alpha_IA = block[IA_sec, 'alpha1']
        self.IAz = -1. * AIA0 * (((1 + self.z_array) / (1 + z0))**alpha_IA) * (0.0134 / self.Dz_nz)
        return 0

    def get_qi_after_IA(self):
        self.qi_wIA = {}
        for jz in range(self.nbins_tot):
            qi_jz = self.qi_gravonly[jz]
            ni_jz = self.nz_source_all[jz]
            qIAz = self.IAz * ni_jz * (1 / self.dchi_dz)
            self.qi_wIA[jz] = qi_jz + qIAz
        return 0

    def get_qiprod_for_kappa3(self, jz1, jz2, jz3):
        # import pdb; pdb.set_trace()
        if self.IA_model == 'old':
            q1q2q3_eff = self.qi_wIA[jz1] * self.qi_wIA[jz2] * self.qi_wIA[jz3]
        if self.IA_model == 'new':
            q1q2q3 = (self.qi_gravonly[jz1] * self.qi_gravonly[jz2] * self.qi_gravonly[jz3])

            q1q2n3 = (self.qi_gravonly[jz1] * self.qi_gravonly[jz2] * self.nz_source_all[jz3])
            q1q3n2 = (self.qi_gravonly[jz1] * self.qi_gravonly[jz3] * self.nz_source_all[jz2])
            q2q3n1 = (self.qi_gravonly[jz2] * self.qi_gravonly[jz3] * self.nz_source_all[jz1])

            n1n2q3 = (self.nz_source_all[jz1] * self.nz_source_all[jz2] * self.qi_gravonly[jz3])
            n1n3q2 = (self.nz_source_all[jz1] * self.nz_source_all[jz3] * self.qi_gravonly[jz2])
            n2n3q1 = (self.nz_source_all[jz2] * self.nz_source_all[jz3] * self.qi_gravonly[jz1])

            n1n2n3 = (self.nz_source_all[jz1] * self.nz_source_all[jz2] * self.nz_source_all[jz3])

            q1q2q3_eff = q1q2q3 + (1. / 3.) * (self.IAz**2 + 2 * self.IAz) * (q1q2n3 + q1q3n2 + q2q3n1) * (
                1 / self.dchi_dz
                ) + (1. / 3.) * (self.IAz**2 + 2 * (self.IAz**3)) * (n1n2q3 + n1n3q2 + n2n3q1) * (
                    (1 / self.dchi_dz)**2
                    ) + (self.IAz**4) * n1n2n3 * ((1 / self.dchi_dz)**3)
        return q1q2q3_eff

    def get_mask_stuff(self):
        if self.do_mask_stuff:
    ...
    def compute_Plz_mat(self):
        ...
        F_l = hp.sphtfunc.pixwin(self.nside_pixwin, lmax=self.lmax)[:self.lmax]
    ...
    def compute_factosum(self, smoothing_scales1, smoothing_scales2):
        ...
        for i, sm in enumerate(zip(smoothing_scales1, smoothing_scales2)):
            ...
            fact1 = -B * (
                scipy.special.eval_legendre(ell + 1, np.cos(sm_rad1)) -
                scipy.special.eval_legendre(ell - 1, np.cos(sm_rad1))
                ) * A1
            fact1[0] = 1 / (
                4 * np.pi
                )  # MINSUP THIS WAS 1, I THINK FACT1*FACT2 SHOULD BE 1/4PI WHEN l=0? check this by plotting and making it smooth
            fact2 = -B * (
                scipy.special.eval_legendre(ell + 1, np.cos(sm_rad2)) -
                scipy.special.eval_legendre(ell - 1, np.cos(sm_rad2))
                ) * A2
            fact2[0] = 1.0
    ...
    def compute_abc_kappa3(self):
        ...
        self.a = (
            1. + ((self.sig8 * Dz_mat)**coeff[5]) * (0.7 * (4. - 2.**self.ns_mat) /
                                                     (1. + 2.**(2. * self.ns_mat + 1)))**0.5 *
            (q_mat * coeff[0])**(self.ns_mat + coeff[1])
            ) / (1. + (q_mat * coeff[0])**(self.ns_mat + coeff[1]))
        self.b = (1. + 0.2 * coeff[2] * (self.ns_mat + 3) * (q_mat * coeff[6])**
                  (self.ns_mat + coeff[7] + 3)) / (1. + (q_mat * coeff[6])**(self.ns_mat + coeff[7] + 3.5))
        self.c = (
            1. + 4.5 * coeff[3] / (1.5 + (self.ns_mat + 3)**4) * (q_mat * coeff[4])**(self.ns_mat + 3 + coeff[8])
            ) / (1 + (q_mat * coeff[4])**(self.ns_mat + 3.5 + coeff[8]))
        self.a[:, 0] = 1.
        self.b[:, 0] = 1.
        self.c[:, 0] = 1.
        return 0
    ...
    def compute_masked_m123_vec(self, smoothing_scales_all3):
        ...
        moments12a = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales1, smoothing_scales2)], abc=a
            )
        moments13a = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales1, smoothing_scales3)], abc=a
            )
        moments23a = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales2, smoothing_scales3)], abc=a
            )

        moments12b = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales1, smoothing_scales2)], abc=b
            )
        moments13b = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales1, smoothing_scales3)], abc=b
            )
        moments23b = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales2, smoothing_scales3)], abc=b
            )

        moments12c = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales1, smoothing_scales2)], abc=c
            )
        moments13c = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales1, smoothing_scales3)], abc=c
            )
        moments23c = self.compute_masked_m12_from_factosum(
            self.fac_to_sum_all[(smoothing_scales2, smoothing_scales3)], abc=c
            )

        smoothing_scales1, smoothing_scales2, smoothing_scales3 = np.array([smoothing_scales1]
                                                                          ), np.array([smoothing_scales2]
                                                                                     ), np.array([smoothing_scales3])
    ...
    def kappa123_vec(self, corr_all, qi1_qi2_qi3, sm1, sm2, sm3):
        try:
            ...
            perm_all = [
                (sm1, sm2, sm3), (sm1, sm3, sm2), (sm2, sm1, sm3), (sm2, sm3, sm1), (sm3, sm2, sm1), (sm3, sm1, sm2)
                ]
    ...
    def execute(self, block):
        ...
        for i in range((self.nsm)):
            for j in range((self.nsm)):
                self.fac_to_sum_all[(self.sm_all[i], self.sm_all[j])
                                   ] = self.compute_factosum(np.array([self.sm_all[i]]), np.array([self.sm_all[j]]))
        ...
        for jid_c2 in range(len(self.id_corr2_unique)):
            ...
            self.corr2_all[(self.sm_all[i], self.sm_all[j])] = self.compute_masked_m12_from_factosum(
                self.fac_to_sum_all[(self.sm_all[i], self.sm_all[j])]
                )
        ...
        for jid_kp2 in range(len(self.id_kp2_unique)):
            ...
            val_jid_kp2 = (mjz1 * mjz2) * self.kappa2_vec(
                self.corr2_all, self.qi_wIA[jz1] * self.qi_wIA[jz2], self.sm_all[i], self.sm_all[j]
                )
            self.kp2_all[jz1, jz2, i, j] = val_jid_kp2
            kp2_all_array.append(val_jid_kp2)
        self.kp2_all_array = np.array(kp2_all_array)

        self.corr3_all = {}
        for jid_c3 in range(len(self.id_corr3_unique)):
            i, j, k = self.id_corr3_unique[jid_c3]
            self.corr3_all[(self.sm_all[i], self.sm_all[j], self.sm_all[k])] = self.compute_masked_m123_vec(
                (self.sm_all[i], self.sm_all[j], self.sm_all[k])
                )
        ...
        for jid_kp3 in range(len(self.id_kp3_unique)):
            ...
            q1q2q3_eff = self.get_qiprod_for_kappa3(jz1, jz2, jz3)
            val_jid_kp3 = (1. / self.fsky) * (mjz1 * mjz2 * mjz3) * self.kappa123_vec(
                self.corr3_all, q1q2q3_eff, self.sm_all[i], self.sm_all[j], self.sm_all[k]
                )
            self.kp3_all[jz1, jz2, jz3, i, j, k] = val_jid_kp3
            kp3_all_array.append(val_jid_kp3)
        self.kp3_all_array = np.array(kp3_all_array)

        block[('gen_moments_theory', 'kp2_all_array')] = self.kp2_all_array
        block[('gen_moments_theory', 'kp3_all_array')] = self.kp3_all_array
        block[('gen_moments_theory', 'kp2_kp3_all_array')] = np.concatenate((self.kp2_all_array, self.kp3_all_array))

        if self.do_save_DV:
            kp_all_theory = {'kp2': self.kp2_all, 'kp3': self.kp3_all}
            if self.isLHS_sample:
                lhs_id = int(block['emusave', 'lhsid'])
                fname = self.saveDV_dir + '/lhs_n' + str(self.nsamp_save_lhs) + '_jr' + str(
                    self.jr_save_lhs
                    ) + '/' + self.saveDV_prefix + 'kappa_all_jlhs' + str(lhs_id)
                save_obj(fname, kp_all_theory)
            elif self.is1P_sample:
                lhs_id = int(block['emusave', 'lhsid'])
                fname = self.saveDV_dir + '/1Pset' + '/' + self.saveDV_prefix + 'kappa_all_jlhs' + str(lhs_id)
                save_obj(fname, kp_all_theory)

            else:
                fname = self.saveDV_dir + '/' + self.saveDV_prefix + 'kappa_all_fid'
                save_obj(fname, kp_all_theory)
